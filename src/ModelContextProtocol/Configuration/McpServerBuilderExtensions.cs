...
public static partial class McpServerBuilderExtensions
{
    ...
    /// <summary>
    /// Adds types marked with the <see cref="McpServerTypeAttribute"/> attribute from the given assembly as tools to the server.
    /// </summary>
    /// <param name="builder">The builder instance.</param>
    ...
    public static IMcpServerBuilder WithToolsFromAssembly(this IMcpServerBuilder builder, Assembly? toolAssembly = null)
    {
        ...
        return builder.WithTools(
            ...
            where t.GetCustomAttribute<McpServerTypeAttribute>() is not null
            select t);
    }
    #endregion
    ...
    /// <exception cref="ArgumentNullException"><paramref name="builder"/> is <see langword="null"/>.</exception>
    /// <remarks>
    /// This method discovers all instance and static methods (public and non-public) on the specified <typeparamref name="TPrompt"/>
    ...
    public static IMcpServerBuilder WithPrompts<[DynamicallyAccessedMembers(
        DynamicallyAccessedMemberTypes.PublicMethods |
        DynamicallyAccessedMemberTypes.NonPublicMethods |
        DynamicallyAccessedMemberTypes.PublicConstructors)] TPrompt>(
        this IMcpServerBuilder builder)
    {
        Throw.IfNull(builder);

        foreach (var promptMethod in typeof(TPrompt).GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.Instance))
        {
            if (promptMethod.GetCustomAttribute<McpServerPromptAttribute>() is not null)
            {
                builder.Services.AddSingleton((Func<IServiceProvider, McpServerPrompt>)(promptMethod.IsStatic ?
                    services => McpServerPrompt.Create(promptMethod, new McpServerPromptCreateOptions() { Services = services }) :
                    services => McpServerPrompt.Create(promptMethod, typeof(TPrompt), new() { Services = services })));
            }
        }
    ...
    /// <summary>
    /// Adds types marked with the <see cref="McpServerTypeAttribute"/> attribute from the given assembly as prompts to the server.
    /// </summary>
    /// <param name="builder">The builder instance.</param>
